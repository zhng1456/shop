...
function showDetail(id,t){
	...
	function search(){
		if(event.keyCode==13){
			...
			window.location="GoodController/selectGoodByName.do?name="+encodeURI(encodeURI(val));
		}
	}
	function change(val){
...
<div class="details">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>${good.type.name}</td>
                                  <td>${good.price }</td>
                                <td align="center">
                                				<input type="button" value="详情" class="btn" onClick="showDetail('${good.id}','${good.name }')"><input type="button" value="修改" class="btn" onClick="editPro('${good.id}')"><input type="button" value="删除" class="btn"onclick="delPro('${good.id}')">
                    ...
                    <div class='page fix'>
							...
							<a href='GoodController/selectPageGood.do?currentPage=1' class='first'>首页</a>
							<a href='GoodController/selectPageGood.do?currentPage=<%=myPage.getBack() %>' class='pre'>上一页</a>
							当前第<span><%=myPage.getCurrentPage() %>/<%=myPage.getAllPage() %></span>页
							<a href='GoodController/selectPageGood.do?currentPage=<%=myPage.getNext() %>' class='next'>下一页</a>
							<a href='GoodController/selectPageGood.do?currentPage=<%=myPage.getAllPage() %>' class='last'>末页</a>
